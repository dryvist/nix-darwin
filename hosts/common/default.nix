...
in
{
  ...
  system = {
    ...
    clusterLinkPrep = {
      ...
      # Shard-sized night wired ceilings, held only when the RDMA link is
      # active (day value restored at link-down). This is the wired-headroom
      # mitigation the 2026-07-12 panic gated clusterMode re-enable on: the
      # laptop keeps a larger unwirable share to protect its ~25 GB GUI
      # working set. UNVALIDATED until the first supervised plug night.
