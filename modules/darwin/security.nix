...
# - Passwordless sudo for `darwin-rebuild` only (unattended converges by
#   automation; the rest of the posture hardening is unchanged)
...
in
{
  ...
  # ==========================================================================
  # Purpose: unattended converges. Anything running as this user can already
  # reach every credential the user holds, so a NOPASSWD converge adds little
  # beyond a root shell on a FileVault-encrypted, LAN-only workstation, and a
  # password prompt mid-converge stalls automation. The grant is the one
  # darwin-rebuild binary only; `activate` is invoked by darwin-rebuild under
  # the same sudo and needs no separate line. Rollback: `darwin-rebuild --rollback`.
  environment.etc = {
