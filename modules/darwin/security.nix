...
let
  ...
  mlxServerLabel = "dev.vllm-mlx.server";
...
in
{
  ...
  #
  # To disable: comment out or remove the relevant entry below.
  environment.etc = {
    "sudoers.d/darwin-rebuild".text = ''
    ...
    # - bootout / kickstart -k target the exact label `dev.vllm-mlx.server`.
    #   These also match the broader `dev.vllm-mlx.*` cluster-ops glob above, but
    ...
    #   already load their own agents. It is anchored to the exact vllm-mlx server
    ...
    "sudoers.d/mlx-serving-restore".text = ''
      # Allow ${userConfig.user.name} to run the vllm-mlx serving-restore ladder
      # without password. Generated by nix-darwin - do not edit manually
