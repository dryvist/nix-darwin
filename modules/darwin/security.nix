...
in
{
  ...
  #   re-apply it at runtime without handing the step back to the user). The
  #   grant is split into two rules pinned to the `off` and `on` action verbs so
  #   it cannot invoke any other `-setnetworkserviceenabled` form; the service
  #   name stays `*` because it is localisable and derived from bridge0 at
  #   runtime (see cluster-link-prep.sh), so it cannot be pinned statically.
  ...
  environment.etc."sudoers.d/cluster-ops".text = ''
    ...
    ${userConfig.user.name} ALL=(ALL) NOPASSWD: /sbin/ifconfig bridge0 *
