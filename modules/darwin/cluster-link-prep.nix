...
in
{
  options.system.clusterLinkPrep = {
    ...
    dayWiredLimitMb = lib.mkOption {
      ...
      description = ''
        Computed day-serving wired ceiling the cluster watcher restores at
        ...
        nix-ai programs.mlx.clusterMode.dayWiredLimitMb (wired in
  ...
  config = lib.mkIf cfg.enable {
    ...
    environment.etc."sudoers.d/cluster-wired-limit" = lib.mkIf (cfg.clusterWiredLimitMb != null) {
      # Deduped: on a host where the cluster ceiling equals the day ceiling
      # (e.g. the headless server, cluster == appleSiliconTunables day value)
      # the two grants collapse to one line instead of a redundant duplicate.
      text =
        let
