...
let
  ...
  # Home directories are made owner-only at activation; the rationale is in the
  # script's own header, next to the code it justifies.
  agentHomePerms = pkgs.writeShellApplication {
    name = "agent-home-perms";
    text = builtins.readFile ./scripts/agent-home-perms.sh;
  };

  agentHomeDirs = map (agent: agent.homeDir) (lib.attrValues agentUsers);
in
{
  users = {
  ...
  environment.etc =
    ...
    // {
      # ======================================================================
      # The operator's ssh-agent does not follow a session into an identity
      # ======================================================================
      # macOS ships SSH_AUTH_SOCK in the default sudoers env_keep, and
      # `sudo -u <agent> -i` does not strip it, so the operator's agent socket
      # path is visible inside every agent session. The socket itself is
      # currently unreachable — its launchd directory is mode 700 and owned by
      # the operator — so this removes a path disclosure and, more usefully, a
      # dependency on that directory mode, which is a macOS implementation
      # detail rather than something this repository controls.
      #
      # `Defaults>user` applies when a command runs AS that user, which is
      # exactly the `sudo -u <agent>` case, and leaves the operator's own sudo
      # environment untouched. env_keep takes precedence over env_delete, so
      # the name is REMOVED from env_keep rather than added to env_delete.
      "sudoers.d/agent-env-hardening".text = ''
        # Generated by nix-darwin - do not edit manually
        ${lib.concatMapStringsSep "\n" (name: ''Defaults>${name} env_keep -= "SSH_AUTH_SOCK"'') (
          lib.attrNames agentUsers
        )}
      '';
    };

  system.activationScripts.postActivation.text = lib.mkAfter ''
    ${agentHomePerms}/bin/agent-home-perms ${lib.escapeShellArgs agentHomeDirs}
  '';
}
